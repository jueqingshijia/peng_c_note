...
当一个数据存放在两个或两个以上的内存单元中，内存单元地址最小的就是这个数据的地址，即变量的地址

例：char* p = &c;    解析：声明一个指针变量p，并将c的地址赋值给p

char：c是char类型，因此p也是char类型；    *：说明p是一个指针变量

*：解引用操作符；     通过变量中存放的地址，找到变量所指的对象

例：*p = 'a';    ==     c = 'a';     这两个是完全等效的，\*p == c == 内存地址
